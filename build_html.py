...
/* DPS = avg damage per shot * shots * Damage Multiplier(ATT) * Attacks/sec(DEX, RoF).
   ...
   Multi-bullet weapons (e.g. Heartsteel Claymore, Arcane Rapier) list 2+
   "Bullet N" damage groups, each with its own Rate of Fire -- that RoF is
   the fraction of attacks (at the weapon's base attack speed) that deal
   that bullet's damage instead of the default swing, not an independent
   attack-speed stream (see docs/decisions/0003-multi-bullet-dps.md). A
   bullet with no listed Rate of Fire (the default swing) gets whatever
   probability is left over from the others. DPS is the sum of each group's
   contribution: avg * shots * DamageMultiplier * (baseAttacksPerSec * RoF%). */
function parseDamageProfile(item) {
  ...
  if (bulletNums.length >= 2) {
    const groups = [];
    let explicitTotal = 0, missingIdx = -1;
    bulletNums.forEach(n => {
      ...
      const rof = rofSeg ? readPct(rofSeg.value) : null;
      if (rof == null) missingIdx = groups.length; else explicitTotal += rof;
      groups.push({ avg, shots, rof });
    });
    if (!groups.length) return null;
    if (missingIdx !== -1) groups[missingIdx].rof = Math.max(0, 100 - explicitTotal);
    if (groups.some(g => g.rof == null)) return null;
    return groups;
  }
